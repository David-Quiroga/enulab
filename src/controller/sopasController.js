...
sopasCtl.mandar = async (req, res) => {
    ...
    try {
        await sql.query(
            'INSERT INTO sopas (nombre, descripcion, precio, porciones, subCategoria,  estado) VALUES (?, ?, ?, ?, ?, ?)',
...
sopasCtl.actualizar = async (req, res) => {
    ...
    if (!idSopa || !nombre || !descripcion || !precio || !porciones || !subCategoria || !estado) {
        return res.status(400).send('Faltan campos obligatorios');
    }

    try {
        const result = await sql.query(
            'UPDATE sopas SET nombre = ?, descripcion = ?, precio = ?, porciones = ?, subCategoria = ?, estado = ? WHERE idSopa = ?',
            [nombre, descripcion, precio, porciones, subCategoria, estado, idSopa]
        );
